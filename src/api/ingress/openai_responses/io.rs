...
use crate::protocol::openai_responses::response_encoder::encode_responses_output;
...
pub(in crate::api) async fn handle_non_streaming(
    ...
    handle_non_streaming_common(
        ...
        INGRESS,
        encode_openai_responses_client_response,
    )
    .await
}
...
fn encode_openai_responses_client_response(
    upstream_response: &CanonicalResponse,
    client_model: &str,
) -> Result<Response, CanonicalError> {
    let client_response = encode_responses_output(upstream_response, client_model)?;
