...
const CONFIG: UriUrlEndpointConfig = UriUrlEndpointConfig {
    ingress: IngressApi::OpenAiChat,
    request_label: "OpenAI Chat request",
    uri_getter: openai_chat_uri_getter,
    url_getter: openai_chat_url_getter,
    rewrite_model_field: true,
};

fn openai_chat_uri_getter(prepared_upstream: &PreparedUpstream) -> Option<&http::Uri> {
    prepared_upstream.openai_chat_uri_parsed()
}

fn openai_chat_url_getter(prepared_upstream: &PreparedUpstream) -> Option<&url::Url> {
    prepared_upstream.openai_chat_url_parsed()
}
